...
{	 
	...
	public void OnSceneGUI()
	{
		if (tweenScript != null)
		{
			if (tweenScript.rectTransform)
			{
                ...
                {
                    Vector3 firstPos = tweenScript.animationParts.PositionPropetiesAnim.StartWorldPos;
                    Vector3 secondPos = tweenScript.animationParts.PositionPropetiesAnim.EndWorldPos;
	...
	void SetStartValues()
	{
		...
		{
			if (tweenScript.IsObjectOpened() ) SetAlphaValue(selectedTransform.transform, 1f);
			else SetAlphaValue(selectedTransform.transform, 0f);
		}
	}
	...
	void SetEndValues()
	{
		...
		if (tweenScript.animationParts.FadePropetiesAnim.IsFadeEnabled())
		{
			if (tweenScript.IsObjectOpened() ) SetAlphaValue(selectedTransform.transform, 0f);
			else SetAlphaValue(selectedTransform.transform, 1f);
		}
	}
